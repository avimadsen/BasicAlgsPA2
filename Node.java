	...
	static WorkSpace doInsert(String key, int value, Node p, int h) {
		...
		if (h == 0) {
			...
			if (key.compareTo(q.child0.guide) <= 0) {
				...
				{
					//here
					q.child0.value = q.child0.value + delta;

					q.child1.value = q.child1.value + delta;
					q.value =0;
				...
				else
				{
					...
					q.child1.value = q.child1.value + delta;
					q.child2.value = q.child2.value + delta;
					q.value =0;

				}
				ws = doInsert(key, value, q.child0, h-1);
				// add value to child 1 and child 2
			}
				...
				//add value to child 0 and child 2 if it exists
			}
			else {
				pos = 2; 
				q.child0.value = q.child0.value + delta;
				...
				//add value to child 0 and child 1
			}
